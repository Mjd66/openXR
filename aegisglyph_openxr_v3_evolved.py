...
class ActionRegistry:
    ...
    def _build_defaults(self) -> None:
        ...
        self.interaction_profiles = [
            InteractionProfile("/interaction_profiles/khr/simple_controller", "baseline portability", ["navigate", "confirm", "haptic_out"]),
            InteractionProfile("/interaction_profiles/oculus/touch_controller", "pcvr/mobile controller family", ["navigate", "confirm", "menu", "haptic_out"]),
            InteractionProfile("/interaction_profiles/ext/hand_interaction_ext", "optical hands", ["confirm", "focus_pose"]),
            InteractionProfile("/interaction_profiles/microsoft/hand_interaction", "hand interaction profile", ["confirm", "focus_pose"]),
        ]
...
class GuidancePlannerModule(Module):
    ...
    def process(self, stage: ModuleStage, ctx: FrameContext) -> None:
        ...
        if ctx.sensor.affect.pain > 0.45 or ctx.sensor.affect.overload > 0.72 or danger > 1.0:
            mode = GuidanceMode.RECOVER
            cue_word = "soften"
        ...
        for frame_index, sensor in enumerate(self.backend.frames(seconds=seconds, hz=hz)):
            ...
            if not quiet:
                ...
                print(
                    ...
                    f"cpu={metrics.total_ms():6.2f}ms  say={lead}"
                )
